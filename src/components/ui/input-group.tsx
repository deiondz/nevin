...
export function InputGroup({
  ...
  return (
    <div
      className={cn(
        "inline-flex h-9 w-full min-w-0 items-center rounded-md border border-input bg-transparent text-base shadow-xs transition-[color,box-shadow] outline-none selection:bg-primary selection:text-primary-foreground has-[input:disabled,textarea:disabled]:pointer-events-none has-[input:disabled,textarea:disabled]:cursor-not-allowed has-[input:disabled,textarea:disabled]:opacity-50 md:text-sm dark:bg-transparent has-[input:focus-visible,textarea:focus-visible]:border-ring has-[input:focus-visible,textarea:focus-visible]:ring-[3px] has-[input:focus-visible,textarea:focus-visible]:ring-ring/50 has-[input[aria-invalid=true],textarea[aria-invalid=true]]:border-destructive has-[input[aria-invalid=true],textarea[aria-invalid=true]]:ring-destructive/20 dark:has-[input[aria-invalid=true],textarea[aria-invalid=true]]:ring-destructive/40 has-[textarea]:h-auto has-data-[align=block-end]:h-auto has-data-[align=block-start]:h-auto has-data-[align=block-end]:flex-col has-data-[align=block-start]:flex-col has-data-[align=inline-start]:**:[input]:ps-2 has-data-[align=inline-end]:**:[input]:pe-2 has-data-[align=block-end]:**:[input]:pt-1.5 has-data-[align=block-start]:**:[input]:pb-1.5 **:[textarea]:min-h-20.5 **:[textarea]:resize-none **:[textarea]:py-[calc(--spacing(3)-1px)] **:[textarea]:max-sm:min-h-23.5 **:[textarea_button]:rounded-[calc(var(--radius-md)-1px)]",
        className,
      )}
